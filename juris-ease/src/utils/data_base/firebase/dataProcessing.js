...
export function createUserByData(data) {
  ...
  const { uid, accessToken, email, phoneNumber, address, name, type, oab, permissions, state } = data;
  const missingFields = [];
  ...
  const user = new User(
    ...
    type,
    oab,
    permissions || {acessAdmin: false, services: true, headlines: true, templates: false, document_generation: false, manege_users: false},
    state || 'Bloqueado'
  );
...
export function createTemplateByData(data) {
  const { id, createdAt, updatedAt, title, rout, contents, keys, typeTermination, numberOfComplaints, typesResponsibilities } = data;
  const missingFields = [];
